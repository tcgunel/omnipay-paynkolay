...
- **The postback is JSON, so `USE_3D` and `AUTO_COMPLETE` arrive as real booleans.** PHP casts `true` to `"1"`, but the gateway hashes the literal `"true"` — a naive `(string)` cast puts the wrong byte in the string and the check fails on every single transaction. The package normalises booleans before hashing.
