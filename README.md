...
The package is built against Paynkolay's documentation at https://paynkolay.com.tr/entegrasyon/ — but it also absorbs the divergences between the published docs and the actual production payloads (notably the postback signature uses a completely different algorithm than every other endpoint). See the [Doc-vs-prod quirks](#doc-vs-prod-quirks-the-package-absorbs) section.
...
| `verifyHash($merchantSecretKey)` | constant-time check of the postback `hashData` field |
...
| | Outgoing requests (`hashDatav2`) | Inbound postback (`hashData`) |
|---|---|---|
| Field separator | `\|` | none (raw concatenation) |
| Hash algorithm | SHA-512 → base64 | SHA-1 → hex-decode → base64 |
| Field name | `hashDatav2` | `hashData` |
